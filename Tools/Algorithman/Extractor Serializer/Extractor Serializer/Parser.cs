...
namespace Extractor_Serializer
{
    public sealed class Parser
    {
        ...
        protected internal sealed class BufferedReader
        {
            ...
            public int ReadCNum(string Label)
            {
                ...
                checked
                {
                    ...
                    {
                        int num = BitConverter.ToInt32(this.Buffer, this.Ptr);
                        num = (int)((long)Math.Round(Math.Round(unchecked((double)num / 1009.0 - 1.0))));
                        this.AddValue(Conversions.ToString(num));
                        this.Ptr += 4;
                        result = num;
                    }
        ...
        private string NewDF;
        private Hashtable Checksums;
        private static Parser.ParserReturns ReturnCode;
        private static Thread pThread = null;
        private static Thread cThread = null;
        private Parser.BufferedReader BR;
        private Hashtable FunctionSets;
        ...
        private static Plugin Output
        {
            ...
            set
            {
                ...
                bool flag = Parser._Output != null;
                if (flag)
                {
                    Parser._Output.ChangePriority -= obj;
                    Parser._Output.Abort -= obj2;
                }
                Parser._Output = value;
                flag = (Parser._Output != null);
                if (flag)
                {
                    Parser._Output.ChangePriority += obj;
                    Parser._Output.Abort += obj2;
        ...
        public string AbortMsg
        {
            get
            {
                return Parser.myAbortMsg;
            }
        }
        [DebuggerNonUserCode]
        private static void __ENCAddToList(object value)
        {
            List<WeakReference> _ENCList = Parser.__ENCList;
            Monitor.Enter(_ENCList);
            checked
            {
                try
                {
                    bool flag = Parser.__ENCList.Count == Parser.__ENCList.Capacity;
                    if (flag)
                    {
                        int num = 0;
                        int arg_3F_0 = 0;
                        int num2 = Parser.__ENCList.Count - 1;
                        int num3 = arg_3F_0;
                        while (true)
                        {
                            int arg_90_0 = num3;
                            ...
                            WeakReference weakReference = Parser.__ENCList[num3];
                            ...
                            if (flag)
                            {
                                ...
                                if (flag2)
                                {
                                    Parser.__ENCList[num] = Parser.__ENCList[num3];
                                }
                                num++;
                            }
                            num3++;
                        }
                        Parser.__ENCList.RemoveRange(num, Parser.__ENCList.Count - num);
                        Parser.__ENCList.Capacity = Parser.__ENCList.Count;
                    }
                    Parser.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
                }
                finally
                {
        ...
        private Plugin.ItemNanoRequirement[] ParseReqs(Parser.ReqsStruc[] RawReqs)
        {
            ...
            checked
            {
                while (true)
                {
                    ...
                    Parser.ReqsStruc reqsStruc = RawReqs[num3];
        ...
        private string[] ParseArgs(string FuncNum, ref bool R)
        {
            ...
            if (flag)
            {
                br.DebugDump("Unhandled Function", Parser.ParserReturns.Failed);
            }
            string[] array = Strings.Split(Conversions.ToString(this.FunctionSets[FuncNum]), ",", -1, CompareMethod.Binary);
